...
def extract_relations(doc, spanbert, internal_relation, entities_of_interest=None, conf=0.7):
    ...
    for sentence in doc.sents:
        if((i+1) % 5 == 0):
            print("        Proccessed {}/{} sentences".format(i+1, num_sentences))
        i = i+1
        ...
        num_relevant_sents = num_relevant_sents + 1
        preds = spanbert.predict(examples)

        for ex, pred in list(zip(examples, preds)):
            relation = pred[0]
            if relation == 'no_relation' or relation != internal_relation:
                continue
            ...
            print("\t\t==========")
    return res, total_extracted, num_relevant_sents
